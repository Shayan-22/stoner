...
const Container = styled(Flex)`
  ...
  padding: 12px;
  align-items: center;
  background: linear-gradient(0deg, rgba(38, 39, 44, 0.4), rgba(39, 38, 44, 0.4)),
    linear-gradient(180deg, #fee133 0%, #fee133 100%);
  ${({ theme }) => theme.mediaQueries.md} {
    padding: 0px;
    background: linear-gradient(180deg, #fee133 0%, #fee133 100%);
  }
`
...
const PhishingWarningBanner: React.FC = () => {
  ...
  const warningText = t("please make sure you're visiting Bharatswap.finance - check the URL carefully.")
  const warningTextAsParts = warningText.split(/(https:\/\/bharatswap.finance)/g)
    ...
    <>
      ...
      {warningTextAsParts.map((text, i) => (
        <Text
          ...
          small
          as="span"
          bold={text === 'BharatSwap'}
          color={text === 'BharatSwap' ? '#FFFFFF' : '#BDC2C4'}
        >
          {text}
        </Text>
      ))}
